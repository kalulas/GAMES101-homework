...
void Application::init() {
  ...
  // Create two ropes 
  ropeEuler = new Rope(Vector2D(0, 200), Vector2D(-400, 200), config.node_count, config.mass,
                       config.ks, {0});
  ropeVerlet = new Rope(Vector2D(0, 200), Vector2D(-400, 200), config.node_count, config.mass,
                        config.ks, {0});
}
